...
import { Search, Bell, ChevronDown, Plus, Calendar, DollarSign, Tag, TrendingUp, TrendingDown } from "lucide-react";
...
const Transactions = () => {
  ...
  const { data: transactions = [], isLoading } = useTransactions(filterCategory, filterType);
  const { data: financialSummary = {totalIncome: 0, totalExpenses: 0}, isLoading: financialSummaryLoading } = useFinancialSummary();
  ...
  const getCategoryInfo = (categoryValue: string) => {
    return categories.find(c => c.id === categoryValue) || categories[categories.length - 1];
  };
